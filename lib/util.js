...
class CtrlCmdUtil {
     ...
     * @returns {Promise.<void,Error>} Promise<なし,エラー>
     */
    sendFileCopy(/*filename*/) {
        /*var options = {
            reqStruct: [
                {name: "filename", type: types.WString}
            ],
            resStruct: [],
            reqData: {
                filename: filename
            },
            param: settings.cmd.SrvFileCopy,
            ver: -1
        };*/

        throw new Error("sendFileCopy is unsupported");
    }

    /**
     * プラグイン一覧取得
     * @param   {Number}                        type タイプ(1: Rename, 2: Write)
     * @returns {Promise.<Array<String>,Error>} Promise<プラグイン一覧,エラー>
     */
    sendEnumPlugIn(type) {
        var options = {
            reqStruct: [
                {name: "type", type: types.UShort}
            ],
            resStruct: [
                {name: "arrPlugin", type: [types.WString]}
            ],
            reqData: {
                type: type
            },
            param: settings.cmd.SrvEnumPlugIn,
            ver: -1
        };

        return this.sendCommand(options).then(data => data.data.arrPlugin);
    }

    /**
     * TVTestチャンネル切り替え用情報取得
     * @param   {Buffer}                 serviceID サービスID
     * @returns {Promise.<Object,Error>} Promise<TVTestチャンネル情報(TvTestChChgInfo),エラー>
     */
    sendGetChangeChannelTVTest(serviceID) {
        var options = {
            reqStruct: [
                {name: "serviceID", type: types.UInt64}
            ],
            resStruct: [
                {name: "tvTestChChgInfo", type: types.TvTestChChgInfo}
            ],
            reqData: {
                serviceID: serviceID
            },
            param: settings.cmd.SrvGetChgChTVTest,
            ver: -1
        };

        return this.sendCommand(options).then(data => data.data.tvTestChChgInfo);
    }
}
