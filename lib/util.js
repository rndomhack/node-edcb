...
class CtrlCmdUtil {
    ...
    /**
     * 番組情報検索
     * @param   {Array<Object>}                 arrSearchKey 検索キー([EpgSearchKeyInfo])
     * @returns {Promise.<Array<Object>,Error>} Promise<番組情報一覧([EpgEventInfo]),エラー>
     */
    sendSearchProgram(arrSearchKey) {
        var options = {
            reqStruct: [
                { name: "arrSearchKey", type: [types.EpgSearchKeyInfo] }
            ],
            resStruct: [
                { name: "arrEpgEventInfo", type: [types.EpgEventInfo] }
            ],
            reqData: {
                arrSearchKey: arrSearchKey
            },
    ...
    sendEnumEpgAutoAdd() {
        var options = {
            ...
            reqData: {},
            param: settings.cmd.SrvEnumEpgAutoAdd,
            ver: -1
        };
